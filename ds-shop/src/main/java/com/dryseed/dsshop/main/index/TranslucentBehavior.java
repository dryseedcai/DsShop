...
import com.dryseed.ds.ui.recycler.RgbValue;
...
public class TranslucentBehavior extends CoordinatorLayout.Behavior<Toolbar> {
    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, Toolbar child, View target, int dx, int dy, int[] consumed) {
        ...
        if (target instanceof SwipeRefreshLayout) {
            RecyclerView recyclerView = (RecyclerView) ((SwipeRefreshLayout) target).getChildAt(0);
            float y = recyclerView.getChildAt(0).getY();
            float sY = recyclerView.getChildAt(0).getScrollY();
            //Log.d("MMM", String.format("y : %s | sY : %s", y, sY));
        }
